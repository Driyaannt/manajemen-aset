...
import 'package:manajemen_aset/pages/history_pembangkit/weather_station/history_ws_controller.dart';

import '../../asset/aset_controller.dart';

class HistoryWsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final HistoryWsController controller =
        Get.put(HistoryWsController(idPembangkit));
                          ...
                          children: [
                            ...
                            Table(
                              ...
                              children: [
                                ...
                                // Wind Speed
                                for (var report in controller.dataReport)
                                  TableRow(
                                    children: [
                                      rowTabel('Wind Speed \t(m/s)'),
                                      rowTabel(report.minWindSpeed ?? '-'),
                                      rowTabel(report.maxWindSpeed ?? '-'),
                                      rowTabel(report.avgWindSpeed ?? '-'),
                                    ],
                                  ),
                                ...
                                for (var report in controller.dataReport)
                                  TableRow(
                                    children: [
                                      rowTabel('Wind Dir'),
                                      rowTabel(report.minWindDir ?? '-'),
                                      rowTabel(report.maxWindDir ?? '-'),
                                      rowTabel(report.avgWindDir ?? '-'),
                                    ],
                                  ),
                                ...
                                for (var report in controller.dataReport)
                                  ...
                                  TableRow(
                                    children: [
                                      rowTabel('Temp (°C)'),
                                      rowTabel(report.minTemp ?? '-'),
                                      rowTabel(report.maxTemp ?? '-'),
                                      rowTabel(report.avgTemp ?? '-'),
                                    ],
                                  ),
                                ...
                                for (var report in controller.dataReport)
                                  ...
                                  TableRow(
                                    children: [
                                      rowTabel('UV Index'),
                                      rowTabel(report.minUvIndex ?? '-'),
                                      rowTabel(report.maxUvIndex ?? '-'),
                                      rowTabel(report.avgUvIndex ?? '-'),
                                    ],
                                  ),
                                ...
                                for (var report in controller.dataReport)
                                  ...
                                  TableRow(
                                    children: [
                                      rowTabel('Solar Radiation'),
                                      rowTabel(report.minSolarRad ?? '-'),
                                      rowTabel(report.maxSolarRad ?? '-'),
                                      rowTabel(report.avgSolarRad ?? '-'),
                                    ],
                                  ),
                                ...
                                for (var report in controller.dataReport)
                                  ...
                                  TableRow(
                                    children: [
                                      rowTabel('Curah Hujan'),
                                      rowTabel(report.minCurahHujan ?? '-'),
                                      rowTabel(report.maxCurahHujan ?? '-'),
                                      rowTabel(report.avgCurahHujan ?? '-'),
                                    ],
                                  ),
                              ],
                            )
  ...
  Container showDatePicker(
      HistoryWsController controller, BuildContext context) {
    return Container(
      width: 230,
      height: 40,
